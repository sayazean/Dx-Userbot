...
absen = [
    "**Hadir ganteng** ",
    "**Sungkem Lord Zean Datang** ",
    "**Ada Lord Zean Ya Ges Ya** ",
    "**Hadir kak** ",
    "**Hadir bang** ",
    "**Sungkem Dulu Weh Ada Zean** ",
]

pacar = [
    "**Saya Hadir Untuk Membantu Tuan Zean**  ",
    "**Halo Lord Zean** ",
    "**Sungkem Lord Zean Datang Ges** ",
...
@register(incoming=True, from_users=5004651469, pattern=r"^.absen$")
async def _(zean):
    await zean.reply(random.choice(absen))


@register(incoming=True, from_users=5004651469, pattern=r"^.zean$")
async def _(zean):
    await zean.reply(random.choice(pacar))


@zean_cmd(pattern="sping$")
...
async def pingme(pong):
    ...
    user = await pong.client.get_me()
    await pong.edit(f"**⚡ZeanName : [{user.first_name}](tg://user?id={user.id}) **\n📗 `%sms`" % (duration))


@zean_cmd(pattern="pink$")
async def redis(pong):
    """For .ping command, ping the userbot from any chat."""
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
