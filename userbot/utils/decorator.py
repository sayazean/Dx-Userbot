    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            zean_reg = sudo_reg = re.compile(pattern)
        else:
            zean_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            zean_reg = re.compile(zean_ + pattern)
        ...
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(
                    **args, outgoing=True, pattern=zean_reg))
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=zean_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
